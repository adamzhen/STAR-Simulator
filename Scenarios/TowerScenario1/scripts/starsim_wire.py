...
OBJECT_NAME   = 'SMAWire_(Nitinol)'  # ENFORCE NO SPACES 
OBJECT_FILE   = 'SMAWire_(Nitinol).stp'
OBJECT_TYPE   = 'wire'  # 'wire' or 'shell'
FCSTD_FILE    = 'TowerScenario1.FCStd'
...
SMA_rad = 0.00157 / 2  # SMA wire radius in meters

WORKING_DIR = f"{STAR_DIR}/Scenarios/{SCENARIO_NAME}"

sys.path.append(f"{WORKING_DIR}/scripts")
from tower_with_star import build_model, assembleTower, addActivationStep
...
# Derived Parameters
SUN_DIR = make_sun_dir(zenith_deg=SUN_ZENITH, azimuth_deg=SUN_AZIMUTH, up_axis='y') # Direction of sunlight in FreeCAD coords
print(f"Sun direction vector (FreeCAD coords): {SUN_DIR}")
...
FREECAD_MACRO      = f"{WORKING_DIR}/scripts/SolarFluxCalcWire.FCMacro"
...
log(f"ABSORPTION_ONLY = {ABSORPTION_ONLY}")
log(f"ABSORPTIVITY_DICT = {ABSORPTIVITY_DICT}")
log(f"OBJECT_TYPE = {OBJECT_TYPE}")
log(f"GEOMETRY_IMPORT = {GEOMETRY_IMPORT}")
log("")
...
def build_model_from_step(model_name, step_name, step_path,
    ...
    surf_all = a.Surface(side1Faces=s1, name=LOAD_SURFACE)
...
for it in range(1, N_ITER + 1):
    ...
    node_data = {
        "nodes": {  1: (0.0675251, 0.27, -0.00597487),
                    2: (0.0675251, 0.81, -0.00597487)},
        "elements": [(1, 2)],
        "radius": SMA_rad
    }
    transfer_data_to_freecad(
        ...
        absorptivity_dict=ABSORPTIVITY_DICT, object_type=OBJECT_TYPE, 
        geometry_import=GEOMETRY_IMPORT, node_data=node_data
    )
    run_freecad_macro(FREECAD_CMD, FREECAD_MACRO, FREECAD_TIMEOUT)
    ...
    if iter_id == 1:
        ...
        #                         part_surface_name=LOAD_SURFACE)
        ...
        mdb.Job(name=job_name, model=MODEL_BASENAME,
               description='Iteration 1', type=ANALYSIS,
